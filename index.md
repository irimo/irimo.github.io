...
* Web サーバサイドエンジニアの Irikida(入木田萌) と申します
* フレンドリーな女性です。場を円滑にする能力があります。「調律力」と形容しています。
 ...
 * 2025.4 東北福祉大学 総合福祉学部（通信教育部）　3年次編入学
 ...
 * スプレッドシートの数式計算
 * 経理系スプレッドシートアプリ
 * 勉強系コンテンツのショート動画切り抜き・SNS投稿
 * 経理系ルーティン業務
 * Illustratorを使用したデータの修正
 * Google Gemini を使用した画像生成・その修正をPhotoshopで行う業務
 * Web系ビジネスアイデアのコンサルタント
...
* Web開発者として一通りできる（アップロード・インストールなど）

### クラウド

* AWS: 業務でひととおり使用経験あり。AWS公式の3日間のスクーリング受講経験あり
* 英語のドキュメントを見てアプリケーションが作れる
...
* 一人でWebサイト（業務用Webツールも含める）の作成ができる。その際、設計の方向性を見るためのディレクターの存在が必要だが、コーディングはすべて自分で行える。
...
* ChatGPT, Gemini, Claude ひととおり触ったことがあります。こだわりはありません。
* コーディング補助は Antigravity, Cursor を使ったことがあります。こちらもこだわりはありません。
* 哲学的な対話など、文系業務をAIで行うことは苦手です。
